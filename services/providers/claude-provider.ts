...
Explain this ${request.language} code for a ${request.level} developer:
...
Format as JSON:
{
  ...
  "relatedTopics": ["array of related topics"]
      ...
      const data: ClaudeResponse = await response.json()
      const explanationText = data.content[0]?.text || '{}'
...
Review and suggest improvements for this ${request.language} code:
...
Provide suggestions in JSON format:
{
  ...
  private buildMessages(request: CodeGenerationRequest): ClaudeMessage[] {
